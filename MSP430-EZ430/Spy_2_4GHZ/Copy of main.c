...
static void dump_rssi()
{
       ...
       for(i=0; i<256; i++)
       {
          ...
          //if (i==0)
          {
               hal_spi_radio_writeReg( MRFI_CC2500_SPI_REG_CHANNR, i); // was k++. wtf? // Channel number. Default spacing is ~200KHz/channel
          }
		hal_spi_radio_cmdStrobe( 0x34 );	// enter Rx mode (not needed except to start autocal)

              //uart0_polled_puts("chan=0x");
          uart0_polled_putHex8(i);
              //uart0_polled_puts("  rssi=0x");
          rssimax = 0;
               for (j=0; j<2; j++)
               {
                   rssi += (hal_spi_radio_readReg(MRFI_CC2500_SPI_REG_RSSI) + 128 -58);
 			if (rssi > rssimax) 
			{
				rssimax = rssi;
			}
           ...
           for (j=0; j<1; j++)
           {
              rssi = (hal_spi_radio_readReg(MRFI_CC2500_SPI_REG_RSSI) + 128 -58); /*^ 0x80); */ // correct for 2's complement
              if (rssi < 0) {rssi = 0;}
              rssi_acc += rssi;

            }
		hal_spi_radio_cmdStrobe( 0x36 );	// enter IDLE mode (not needed except for autocal)
              //uart0_polled_putHex8( rssimax & 0x00FF );
              uart0_polled_putHex8( (rssi_acc >> 0));
              uart0_polled_putcrlf();
        }
       //}
}
